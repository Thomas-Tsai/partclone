...
RUN apt-get update
RUN apt-get install -y f2fs-tools hfsprogs exfatprogs gddrescue libfuse-dev jfsutils reiser4progs hfsprogs hfsplus reiserfsprogs btrfs-progs wget gnupg2 git libxxhash-dev libisal-dev zlib1g-dev libzstd-dev
...
WORKDIR /partclone
RUN ./autogen && ./configure --enable-fs-test --enable-feature-test --enable-ncursesw --enable-xxhash --enable-isal --enable-extfs --enable-ntfs --enable-fat --enable-exfat --enable-hfsp --enable-apfs --enable-btrfs --enable-minix --enable-f2fs --enable-reiser4 --enable-xfs && make && make install
