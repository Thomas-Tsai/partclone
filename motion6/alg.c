...
struct coord alg_locate_center_size(struct images *imgs, int width, int height)
{
...
void alg_draw_location(struct coord *cent, struct images *imgs, int width, int height, unsigned char *new, int mode)
{
	unsigned char *out=imgs->out;
	int x, y;
...
/*
	...
	overlaying.
*/
char alg_diff_fast (struct context *cnt, int max_n_changes, unsigned char *new)
{
	struct images *imgs=&cnt->imgs;
...
int alg_switchfilter (struct context *cnt, int diffs, struct coord *cent, unsigned char *newimg)
{
...
//	printf("%d %d %d %d  %d\n", lines, vertlines, linediff, lines-vertlines, diffs);
	if (vertlines>cnt->imgs.height/10 && lines<vertlines/3 &&
		...
		if (cnt->conf.text_changes) {
			...
			draw_text(newimg, cnt->imgs.width-10, 20, cnt->imgs.width, cnt->imgs.height, tmp, cnt->conf.text_double);
		}
		return diffs;
	}
	return 0;
