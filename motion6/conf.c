...
struct context **conf_cmdparse(struct context **cnt, char *cmd, char *arg1)
{
	...
	motion_log(cnt[0], LOG_ERR, 1, "Unknown config option \"%s\"",cmd);
...
void conf_print(struct context **cnt)
{
	...
	for (thread=0; cnt[thread]; thread++) {
		motion_log(cnt[0], LOG_INFO, 1, "Writing config file to %s",cnt[thread]->conf_filename);
		conffile=myfopen(cnt[0] ,cnt[thread]->conf_filename, "w");
		if (!conffile)
			continue;
		...
		for (i=0; config_params[i].param_name; i++) {
			...
			if (val) {
				...
				if (strncmp(config_params[i].param_name, "text", 4) || strncmp(val, " ", 1))
...
struct context ** conf_load (struct context **cnt)
{
	...
	if (!fp){      /* Commandline didn't work, try current dir */
		if (cnt[0]->conf_filename[0])
			motion_log(cnt[0], -1, 1, "Configfile %s not found - trying defaults.", filename);
	...
	if (!fp) {     /* specified file does not exist... try default file */
		...
		if (!fp) {
			...
			if (!fp)        /* there is no config file.... use defaults */
				motion_log(cnt[0], -1, 1, "could not open configfile %s",filename);
		}
	}
	...
	if (fp) {
		...
		motion_log(cnt[0], LOG_INFO, 0, "Processing thread 0 - config file %s",filename);
...
char *mystrdup(char *from)
{
	...
	if (from == NULL || !strlen(from)) {
		...
		tmp = (char *)mymalloc(NULL, stringlength + 1);
...
static char *print_thread(struct context **cnt, char **str, int parm, int threadnr)
{
	...
	retval[0]=0;
	while (cnt[++i]) {
		retval=myrealloc(cnt[0] ,retval, strlen(retval)+strlen(cnt[i]->conf_filename)+10, "print_thread");
...
static struct context **config_thread(struct context **cnt, char *str, int val)
{
	...
	fp=fopen(str, "r");
	if (!fp) {
		motion_log(cnt[0], LOG_ERR, 1, "Thread config file %s not found",str);
		return cnt;
	}
	 ...
	 */
	cnt=myrealloc(cnt[0], cnt, sizeof(struct context *)*(i+2), "config_thread");

	/* Now malloc space for an additional context structure for thread nr. i */
	cnt[i]=mymalloc(cnt[0], sizeof(struct context));
...
#if 0		/* we will assign threadnr when we launch the threads */
	...
	motion_log(cnt[0], LOG_INFO, 0, "Processing config file %s", str);
