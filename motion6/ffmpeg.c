...
void ffmpeg_put_frame(struct context *, struct ffmpeg *, AVFrame *);
void ffmpeg_cleanups(struct ffmpeg *);
AVFrame *ffmpeg_prepare_frame(struct context *, struct ffmpeg *, unsigned char *, 
 ...
 */
int file_open_append(URLContext *h, const char *filename, int flags)
{
	const char *colon;
	int access, fd;
	...
	if (flags & URL_RDWR) {
		...
		access = O_CREAT | O_APPEND | O_WRONLY;
	} else {
		access = O_RDONLY;
	}
 ...
 */
AVOutputFormat *get_oformat(struct context *cnt, const char *codec, char *filename)
{
	const char *ext;
	...
	if ((strcmp(codec, TIMELAPSE_CODEC) == 0)
		...
		motion_log(cnt, LOG_ERR, 0, "*** mpeg1 support for normal videos has been disabled ***");
		return NULL;
#endif
		...
		motion_log(cnt, LOG_ERR, 0, "ffmpeg_video_codec option value %s is not supported", codec);
		return NULL;
	}

	if (!of) {
		motion_log(cnt, LOG_ERR, 0, "Could not guess format for %s", codec);
		return NULL;
	}
 ...
 */
struct ffmpeg *ffmpeg_open(struct context *cnt, char *ffmpeg_video_codec, 
                           char *filename, unsigned char *y, unsigned char *u, 
                           unsigned char *v, int width, int height, int rate, 
                           int bps, int vbr)
{
	AVCodecContext *c;
	AVCodec *codec;
	 ...
	 * ffmpeg is freed.
	 */
	ffmpeg = mymalloc(cnt, sizeof(struct ffmpeg));
	...
	if (!ffmpeg->oc) {
		motion_log(cnt, LOG_ERR, 1, "Memory error while allocating output media context");
		ffmpeg_cleanups(ffmpeg);
		return (NULL);
	}

	/* Setup output format */
	ffmpeg->oc->oformat = get_oformat(cnt, ffmpeg_video_codec, filename);
	...
	if (ffmpeg->oc->oformat->video_codec != CODEC_ID_NONE) {
		...
		if (!ffmpeg->video_st) {
			motion_log(cnt, LOG_ERR, 1, "av_new_stream - could not alloc stream");
			ffmpeg_cleanups(ffmpeg);
			return (NULL);
		}
	} else {
		/* We did not get a proper video codec. */
		motion_log(cnt, LOG_ERR, 0, "Failed to obtain a proper video codec");
		ffmpeg_cleanups(ffmpeg);
		return (NULL);
	}
	...
	if (av_set_parameters(ffmpeg->oc, NULL) < 0) {
		motion_log(cnt, LOG_ERR, 0, "ffmpeg av_set_parameters error: Invalid output format parameters");
		ffmpeg_cleanups(ffmpeg);
		return (NULL);
	}
	...
	if (!codec) {
		motion_log(cnt, LOG_ERR, 1, "Codec not found");
		ffmpeg_cleanups(ffmpeg);
		return (NULL);
	}
	...
	if (avcodec_open(c, codec) < 0) {
		motion_log(cnt, LOG_ERR, 1, "avcodec_open - could not open codec");
		ffmpeg_cleanups(ffmpeg);
		return (NULL);
	}
	...
	if (!(ffmpeg->oc->oformat->flags & AVFMT_RAWPICTURE)) {
		...
		ffmpeg->video_outbuf = mymalloc(cnt, ffmpeg->video_outbuf_size);
	}

	/* allocate the encoded raw picture */
	ffmpeg->picture = avcodec_alloc_frame();
	if (!ffmpeg->picture) {
		motion_log(cnt, LOG_ERR, 1, "avcodec_alloc_frame - could not alloc frame");
		ffmpeg_cleanups(ffmpeg);
		return (NULL);
	}
	...
	if (!(ffmpeg->oc->oformat->flags & AVFMT_NOFILE)) {
		...
		if (url_fopen(&ffmpeg->oc->pb, file_proto, URL_WRONLY) < 0) {
			...
			if (errno == ENOENT) {
				...
				if (url_fopen(&ffmpeg->oc->pb, file_proto, URL_WRONLY) < 0) {
					motion_log(cnt, LOG_ERR, 1, "url_fopen - error opening file %s",filename);
					ffmpeg_cleanups(ffmpeg);
					return (NULL);
				}
				/* Permission denied */
				...
				motion_log(cnt, LOG_ERR, 1, "Error opening file %s", filename);
				ffmpeg_cleanups(ffmpeg);
				return (NULL);
			}
		}
...
void ffmpeg_put_image(struct context *cnt, struct ffmpeg *ffmpeg) 
{
	ffmpeg_put_frame(cnt, ffmpeg, ffmpeg->picture);
}

/* Puts an arbitrary picture defined by y, u and v. */
void ffmpeg_put_other_image(struct context *cnt, struct ffmpeg *ffmpeg, 
                            unsigned char *y, unsigned char *u, 
                            unsigned char *v)
{
	AVFrame *picture;
	/* allocate the encoded raw picture */
	picture = ffmpeg_prepare_frame(cnt, ffmpeg, y, u, v);

	if (picture) {
		ffmpeg_put_frame(cnt, ffmpeg, picture);
		free(picture);
	}
}
 ...
 */
void ffmpeg_put_frame(struct context *cnt, struct ffmpeg *ffmpeg, AVFrame *pic)
{
	int out_size, ret;
	...
	if (ret != 0) {
		motion_log(cnt, LOG_ERR, 1, "Error while writing video frame");
		return;
	}
}
 ...
 */
AVFrame *ffmpeg_prepare_frame(struct context *cnt, struct ffmpeg *ffmpeg, 
                              unsigned char *y, unsigned char *u, 
                              unsigned char *v)
{
	AVFrame *picture;
	...
	if (!picture) {
		motion_log(cnt, LOG_ERR, 1, "Could not alloc frame");
		return NULL;
	}
