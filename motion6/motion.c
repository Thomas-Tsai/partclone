 ...
 *
 * Returns: adjusted number of different pixels (possibly 0)
 */
static int motion_detected(struct context *cnt, int diffs, int dev, int devpipe, int devmpipe, unsigned char *newimg)
{
	...
	if (diffs) {
		...
		event(cnt, EVENT_MOTION, NULL, NULL, NULL, cnt->currenttime);
	}
	...
	if (cnt->event_nr!=cnt->prev_event) {
		...
		event(cnt, EVENT_FIRSTMOTION, newimg, NULL, NULL, cnt->currenttime);
		cnt->prev_event=cnt->event_nr;
		cnt->eventtime=cnt->currenttimep;
		
		if (cnt->conf.setup_mode)
			motion_log(cnt, -1, 0, "Motion detected - starting event %d", cnt->event_nr);
	...
	if (cnt->shots < conf->frame_limit && cnt->currenttimep-cnt->lastshot >= conf->mingap )	{
		...
		event(cnt, EVENT_IMAGE_DETECTED, newimg, NULL, NULL, cnt->currenttime);
		...
		if (conf->webcam_motion && !conf->setup_mode && cnt->shots !=1)
			event(cnt, EVENT_WEBCAM, newimg, NULL, NULL, cnt->currenttime);
		cnt->preview_shot=0;
	}
...
static void *motion_loop(void *arg)
{
	...
	motion_log(cnt, LOG_DEBUG, 0, "Thread started");
	...
	rolling_average_data=mymalloc(cnt, sizeof(long int)*rolling_average_limit);
	...
	if (cnt->video_dev==-1) {
		motion_log(cnt, LOG_ERR, 0, "Capture error calling vid_start");
		motion_log(cnt, -1 , 0, "Thread finishing...");
		exit(1);
	}
	cnt->imgs.image_ring_buffer = mymalloc(cnt, cnt->imgs.size);
	memset(cnt->imgs.image_ring_buffer, 0, cnt->imgs.size);       /* initialize to zero */
	cnt->imgs.ref = mymalloc(cnt, cnt->imgs.size);
	cnt->imgs.out = mymalloc(cnt, cnt->imgs.size);
	cnt->imgs.image_virgin = mymalloc(cnt, cnt->imgs.size);
	cnt->imgs.smartmask = mymalloc(cnt, cnt->imgs.motionsize);
	cnt->imgs.smartmask_final = mymalloc(cnt, cnt->imgs.motionsize);
	cnt->imgs.smartmask_buffer = mymalloc(cnt, cnt->imgs.motionsize*sizeof(int));
	cnt->imgs.labels = mymalloc(cnt, cnt->imgs.motionsize*sizeof(cnt->imgs.labels));
	cnt->imgs.labelsize = mymalloc(cnt, (cnt->imgs.motionsize/2+1)*sizeof(cnt->imgs.labelsize));
	cnt->imgs.timestamp = mymalloc(cnt, sizeof(time_t));
	cnt->imgs.shotstamp = mymalloc(cnt, sizeof(int));
	...
	for (i = 0; i < 10; i++) {
		if (vid_next(cnt, cnt->imgs.image_ring_buffer) == 0)
			break;
		SLEEP(2,0);
	}
	if (i >= 10) {
		motion_log(cnt, LOG_ERR, 0, "Error capturing first image");
		motion_log(cnt, -1, 0, "Thread finishing...");
		exit(1);
	}
	...
	if (cnt->conf.vidpipe) {
		if (cnt->conf.setup_mode)
			motion_log(cnt, -1, 0, "Opening video loopback device for normal pictures");
		/* vid_startpipe should get the output dimensions */
		cnt->pipe=vid_startpipe(cnt, cnt->conf.vidpipe, cnt->imgs.width, cnt->imgs.height, cnt->imgs.type);
		if (cnt->pipe < 0) {
			motion_log(cnt, LOG_ERR, 0, "Failed to open video loopback");
			motion_log(cnt, -1, 0, "Thread finishing...");
			exit(1);
		}
	}
	if (cnt->conf.motionvidpipe) {
		if (cnt->conf.setup_mode)
			motion_log(cnt, -1, 0, "Opening video loopback device for motion pictures");
		/* vid_startpipe should get the output dimensions */
		cnt->mpipe=vid_startpipe(cnt, cnt->conf.motionvidpipe, cnt->imgs.width, cnt->imgs.height, cnt->imgs.type);
		if (cnt->mpipe < 0) {
			motion_log(cnt, LOG_ERR, 0, "Failed to open video loopback");
			motion_log(cnt, -1, 0, "Thread finishing...");
			exit(1);
	...
	if(cnt->conf.mysql_db) {
		cnt->database=(MYSQL *) mymalloc(cnt, sizeof(MYSQL));
		...
		if (!mysql_real_connect(cnt->database, cnt->conf.mysql_host, cnt->conf.mysql_user,
			...
			motion_log(cnt, LOG_ERR, 0, "Cannot connect to MySQL database %s on host %s with user %s", cnt->conf.mysql_db, cnt->conf.mysql_host, cnt->conf.mysql_user);
			motion_log(cnt, LOG_ERR, 0, "MySQL error was %s", mysql_error(cnt->database));
			motion_log(cnt, -1, 0, "Thread finishing...");
			exit(1);
		}
	}
	...
	if (cnt->conf.pgsql_db) {
		...
		if (PQstatus(cnt->database_pg) == CONNECTION_BAD) {
			motion_log(cnt, LOG_ERR, 0, "Connection to PostgreSQL database '%s' failed: %s",
	...
	if (cnt->conf.mask_file) {
		if ((picture=fopen(cnt->conf.mask_file, "r"))) {
			...
			motion_log(cnt, LOG_ERR, 1, "Error opening mask file %s", cnt->conf.mask_file);
			/* Try to write an empty mask file to make it easier
			   for the user to edit it */
			put_fixed_mask(cnt, cnt->conf.mask_file);
		}
		if (!cnt->imgs.mask) {
			motion_log(cnt, LOG_ERR, 0, "Failed to read mask image. Mask feature disabled.");
		} else {
			if (cnt->conf.setup_mode)
				motion_log(cnt, -1, 0, "Maskfile \"%s\" loaded.",cnt->conf.mask_file);
		}
	} else
		cnt->imgs.mask=NULL;
	...
	if (cnt->conf.webcam_port) {
		if ( webcam_init(cnt) == -1 ) {
			motion_log(cnt, LOG_ERR, 1, "Problem enabling stream server");
			cnt->finish=1;
			cnt->makemovie=0;
		}
		motion_log(cnt, LOG_DEBUG, 0, "Started stream webcam server in port %d", cnt->conf.webcam_port);
	}
	...
	while (!cnt->finish || cnt->makemovie) {
		...
		if (cnt->precap_nr != frame_buffer_size) {
			...
			if (frame_buffer_size > cnt->precap_nr || frame_buffer_size == cnt->precap_cur) {
				...
				tmp=mymalloc(cnt, cnt->imgs.size*(1+frame_buffer_size));
				tmp2=mymalloc(cnt, sizeof(time_t)*(1+frame_buffer_size));
				tmp3=mymalloc(cnt, sizeof(int)*(1+frame_buffer_size));
		 ...
		 */
		localtime_r(&cnt->currenttimep, cnt->currenttime);
		...
		*(cnt->imgs.shotstamp+cnt->precap_cur)=cnt->shots;
		...
		/* newimg now points to the current image and precap_cur incremented pointing to the
		...
		if (vid_return_code == 0) {
			...
			if (cnt->missing_frame_counter >= MISSING_FRAMES_TIMEOUT * cnt->conf.frame_limit)
				...
				motion_log(cnt, LOG_ERR, 0, "Video signal re-acquired");
			...
			if (vid_return_code < 0) {
				...
				motion_log(cnt, LOG_ERR, 0, "Video device fatal error - terminating camera thread");
				break;
			} else {
				if (debug_level)
					motion_log(cnt, -1, 0, "vid_return_code %d", vid_return_code);
				/* Non fatal error */
				/* First missed frame - store timestamp */
				if (!cnt->missing_frame_counter)
					cnt->connectionlosttime = cnt->currenttimep;
					
				/* increase missing_frame_counter
				 * The first MISSING_FRAMES_TIMEOUT seconds we copy previous virgin image
				 * After 5 seconds we put a grey error image in the buffer
				...
				if (++cnt->missing_frame_counter < (MISSING_FRAMES_TIMEOUT * cnt->conf.frame_limit)) {
					...
					mystrftime(cnt, tmpout, sizeof(tmpout), tmpin, &tmptime);
					draw_text(newimg, 10, 20 * text_size_factor, cnt->imgs.width,
					          cnt->imgs.height, tmpout, cnt->conf.text_double);
					
					/* Write error message only once */
					if (cnt->missing_frame_counter == MISSING_FRAMES_TIMEOUT * cnt->conf.frame_limit)
						motion_log(cnt, LOG_ERR, 0, "Video signal lost - Adding grey image");
				}
			}
		} 
		...
		if (cnt->conf.lightswitch) {
			if (alg_lightswitch(cnt, cnt->diffs)) {
				if (cnt->conf.setup_mode)
					motion_log(cnt, -1, 0, "Lightswitch detected");
				if (cnt->moved<5)
					cnt->moved = 5;
				cnt->diffs = 0;
			}
		...
		/* NOTE! Here we start adding text to the picture buffer.
		 * You should not do any further motion detection features after this
		 * since we do not want the text display to have influence on motion
		 * detection.
		 */
		...
		if (cnt->conf.text_changes) {
			...
			draw_text(newimg, cnt->imgs.width - 10, 10, cnt->imgs.width, cnt->imgs.height, tmp, cnt->conf.text_double);
		}
		...
		if (cnt->conf.setup_mode) {
			...
			draw_text(cnt->imgs.out, cnt->imgs.width - 10, cnt->imgs.height - 30 * text_size_factor, cnt->imgs.width, cnt->imgs.height, tmp, cnt->conf.text_double);
			sprintf(tmp, "THREAD %d SETUP", cnt->threadnr);
			draw_text(cnt->imgs.out, cnt->imgs.width - 10, cnt->imgs.height - 10 * text_size_factor, cnt->imgs.width, cnt->imgs.height, tmp, cnt->conf.text_double);
		}

		/* Add text in lower left corner of the pictures */
		if (cnt->conf.text_left) {
			char tmp[PATH_MAX];
			mystrftime(cnt, tmp, sizeof(tmp), cnt->conf.text_left, cnt->currenttime);
			draw_text(newimg, 10, cnt->imgs.height - 10 * text_size_factor, cnt->imgs.width, cnt->imgs.height, tmp, cnt->conf.text_double);
		}

		/* Add text in lower right corner of the pictures */
		if (cnt->conf.text_right) {
			char tmp[PATH_MAX];
			mystrftime(cnt, tmp, sizeof(tmp), cnt->conf.text_right, cnt->currenttime);
			draw_text(newimg, cnt->imgs.width - 10, cnt->imgs.height - 10 * text_size_factor, cnt->imgs.width, cnt->imgs.height,  tmp, cnt->conf.text_double);
		}
		...
		if (((cnt->currenttimep - cnt->lasttime >= cnt->conf.gap) && cnt->conf.gap > 0) || cnt->makemovie) {
			if (cnt->event_nr == cnt->prev_event || cnt->makemovie) {
				...
				localtime_r(&cnt->eventtime, &tmptime);
				event(cnt, EVENT_ENDMOTION, NULL, NULL, &tmptime, cnt->currenttime);
				...
				if (cnt->conf.setup_mode)
					motion_log(cnt, -1, 0, "End of event %d", cnt->event_nr);
		...
		if ( (cnt->conf.snapshot_interval > 0 && cnt->shots==0 &&
			...
			event(cnt, EVENT_IMAGE_SNAPSHOT, newimg, NULL, NULL, cnt->currenttime);
			cnt->snapshot=0;
		}
		...
		if (cnt->conf.timelapse) {
			...
			if (cnt->currenttime->tm_min == 0 && (time_current_frame % 60 < time_last_frame % 60) && cnt->shots == 0) {
				...
				/* If we are daily, raise timelapseend event at midnight */ 
					...
					if (cnt->currenttime->tm_hour == 0)
						event(cnt, EVENT_TIMELAPSEEND, NULL, NULL, NULL, cnt->currenttime);
				}
				...
				/* If invalid we report in syslog once and continue in manual mode */
					...
					motion_log(cnt, LOG_ERR, 0, "Invalid timelapse_mode argument '%s'",
					           cnt->conf.timelapse_mode);
					motion_log(cnt, LOG_ERR, 0, "Defaulting to manual timelapse mode");
			...
			if (cnt->shots == 0 &&
				...
				event(cnt, EVENT_TIMELAPSE, newimg, NULL, NULL, cnt->currenttime);
		}
		 ...
		 */
		if (cnt->conf.setup_mode) {
			event(cnt, EVENT_IMAGE, cnt->imgs.out, NULL, &cnt->pipe, cnt->currenttime);
			event(cnt, EVENT_WEBCAM, cnt->imgs.out, NULL, NULL, cnt->currenttime);
		} else {
			event(cnt, EVENT_IMAGE, newimg, NULL, &cnt->pipe, cnt->currenttime);
			if (!cnt->conf.webcam_motion || cnt->shots == 1)
				event(cnt, EVENT_WEBCAM, newimg, NULL, NULL, cnt->currenttime);
		}
		
		event(cnt, EVENT_IMAGEM, cnt->imgs.out, NULL, &cnt->mpipe, cnt->currenttime);
		...
		if (cnt->shots==0){
			if (strcasecmp(cnt->conf.output_normal,"on") == 0)
				...
				if (!cnt->imgs.preview_buffer){
					cnt->imgs.preview_buffer=mymalloc(cnt ,cnt->imgs.size);
					if (cnt->conf.setup_mode)
						motion_log(cnt, -1, 0, "Preview buffer allocated");
				}
			}
			else
	...
	motion_log(cnt, -1, 0, "Thread exiting");
	if (!cnt->finish)
		motion_log(cnt, LOG_ERR, 1, "Somebody stole the video device, lets hope we got his picture");
 ...
 * Returns: nothing
 */
static void become_daemon(struct context *cnt)
{
	int i;
	...
	if (fork()) {
		motion_log(cnt, -1, 0, "Motion going to daemon mode");
		exit(0);
	}
	...
	if (chdir("/")) {
		motion_log(cnt, LOG_ERR, 1, "Could not change directory");
	}
...
static void cntlist_create(int argc, char *argv[])
{
	 ...
	 */
	cnt_list=mymalloc(NULL, sizeof(struct context *)*2);
	
	/* Now we reserve room for thread 0's context structure and let cnt_list[0] point to it */
	cnt_list[0]=mymalloc(NULL, sizeof(struct context));
 ...
 * Returns: nothing
 */
void motion_startup(int daemonize, int argc, char *argv[])
{
	...
	if (daemonize) {
		...
		if (cnt_list[0]->daemon && cnt_list[0]->conf.setup_mode==0) {
			become_daemon(cnt_list[0]);
			motion_log(cnt_list[0], LOG_INFO, 0, "Motion running as daemon process");
			if (cnt_list[0]->conf.low_cpu) {
				motion_log(cnt_list[0], LOG_INFO, 0, "Capturing %d frames/s when idle",
...
int main (int argc, char **argv)
{
	...
	if(cnt_list[0]->conf.setup_mode)
		motion_log(cnt_list[0], -1, 0, "Motion running in setup mode.");
	...
	do {
		...
		for (i = 1; cnt_list[i]; i++) {
			...
			while (cnt_list[++j]) {
				if (cnt_list[j]->conf.webcam_port == webcam_port && webcam_port != 0) {
					...
					motion_log(cnt_list[j], LOG_ERR, 0,
		...
		for (i = cnt_list[1] != NULL ? 1 : 0; cnt_list[i]; i++) {
			if (cnt_list[0]->conf.setup_mode)
				motion_log(cnt_list[i], -1, 0, "Thread device: %s input %d",
			...
			if ( strcmp(cnt_list[i]->conf_filename,"") )
				motion_log(cnt_list[i], LOG_INFO, 0, "Thread is from %s", cnt_list[i]->conf_filename );	
		...
		if (cnt_list[0]->conf.setup_mode)
			motion_log(cnt_list[0], -1, 0,"Waiting for threads to finish, pid: %d", getpid());
		...
		if (cnt_list[0]->conf.setup_mode)
			motion_log(cnt_list[0], LOG_DEBUG, 0, "Threads finished");
	...
	motion_log(cnt_list[0], LOG_INFO, 0, "Motion terminating");
 ...
 *
 *   NOTE: Kenneth Lavrsen change printing of size_t types so instead of using 
 ...
 * Parameters:
 *
 *   cnt    - current thread's context structure (for error logging)
 *   nbytes - no. of bytes to allocate
 *
 * Returns: a pointer to the allocated memory
 */
void * mymalloc(struct context *cnt, size_t nbytes)
{
	void *dummy = malloc(nbytes);
	if (!dummy) {
		motion_log(cnt, LOG_EMERG, 1, "Could not allocate %llu bytes of memory!", (unsigned long long)nbytes);
		exit(1);
	}
 ...
 */
int create_path(struct context *cnt, char *path)
{
	char *start;
	...
	while(start) {
		...
		if (mkdir(buffer, mode) == -1 && errno != EEXIST) {
			motion_log(cnt, LOG_ERR, 1, "Problem creating directory %s", buffer);
			free(buffer);
			return -1;
		}
...
FILE * myfopen(struct context *cnt, char *path, char *mode)
{
	...
	if (!dummy)
	{
		 ...
		 */
		motion_log(cnt, LOG_ERR, 1, "Error opening file %s with mode %s", path, mode);
 ...
 * 	                Nothing
 */
void motion_log (struct context *cnt, int level, int errno_flag,
                 const char *fmt, ...)
{
	int errno_save, n;
	char buf[1024];
	...
	n = snprintf(buf, sizeof(buf), "[%d] ", cnt ? cnt->threadnr : 0);
