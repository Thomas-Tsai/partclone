 ...
 * Returns nothing
 */
void put_ppm_bgr24_file(FILE *picture, unsigned char *image, int width, int height)
{
	int x, y;
 ...
 */ 
int put_picture_memory(struct context *cnt, unsigned char* dest_image, int image_size, unsigned char *image, int quality)
{
...
void put_picture(struct context *cnt, char *file, unsigned char *image, int ftype)
{
	...
	if (!picture) {
		...
		if (errno ==  EACCES) {
			...
			motion_log(cnt, LOG_ERR, 1, "Can't write picture to file %s", file);
			return;
		}
	}
...
unsigned char *get_pgm(struct context *cnt, FILE *picture, int width, int height)
{
	int x=0 ,y=0, maxval;
	char line[256];
	unsigned char *image;
	...
	if (!fgets(line, 255, picture)) {
		motion_log(cnt, LOG_ERR, 1, "Could not read from ppm file");
		return NULL;
	}
	
	if (strncmp(line, "P5", 2)) {
		motion_log(cnt, LOG_ERR, 1, "This is not a ppm file, starts with '%s'", line);
		return NULL;
	}
	...
	if (sscanf(line, "%d %d", &x, &y)!=2) {
		motion_log(cnt, LOG_ERR, 1, "Failed reading size in pgm file");
		return NULL;
	}
	
	if (x!=width || y!=height) {
		motion_log(cnt, LOG_ERR, 1, "Wrong image size %dx%d% should be %dx%d", x, y, width, height);
		return NULL;
	}
	/* Maximum value */
	line[0]='#';
	...
	if (sscanf(line, "%d", &maxval)!=1) {
		motion_log(cnt, LOG_ERR, 1, "Failed reading maximum value in pgm file");
		return NULL;
	}
	
	/* read data */
	
	image=mymalloc(cnt, width*height);
	
	for (y=0; y<height; y++) {
		if (fread(&image[y*width], 1, width, picture)!=width)
			motion_log(cnt, LOG_ERR, 1, "Failed reading image data from pgm file");
...
void put_fixed_mask(struct context *cnt, char *file)
{
	...
	if (!picture) {
		...
		if (errno ==  EACCES) {
			...
			/* If target dir is temporarily unavailable we may survive */
			motion_log(cnt, LOG_ERR, 1, "can't write mask file %s", file);
		}
		return;
	}
	...
	if (fwrite(cnt->imgs.out, cnt->conf.width, cnt->conf.height, picture) != cnt->conf.height) {
		motion_log(cnt, LOG_ERR, 1, "Failed writing default mask as pgm file");
		return;
	}
	
	fclose(picture);

	motion_log(cnt, LOG_ERR, 0, "Creating empty mask %s",cnt->conf.mask_file);
	motion_log(cnt, LOG_ERR, 0, "Please edit this file and re-run motion to enable mask feature");
}
...
void preview_best(struct context *cnt)
{
	...
	if(cnt->preview_max){
		...
		mystrftime(cnt, filename, sizeof(filename), jpegpath, cnt->currenttime);
