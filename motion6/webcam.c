 ...
 */
int http_bindsock(struct context *cnt, int port, int local)
{
	int sl, optval=1;
	struct sockaddr_in6 sin;

	if ((sl=socket(PF_INET6, SOCK_STREAM, 0))<0) {
		motion_log(cnt, LOG_ERR, 1, "socket()");
		return -1;
	}
	...
	if (bind(sl, (struct sockaddr *)&sin, sizeof(struct sockaddr_in6))==-1) {
		motion_log(cnt, LOG_ERR, 1, "bind()");
		close(sl);
		return -1;
	}

	if (listen(sl, DEF_MAXWEBQUEUE)==-1) {
		motion_log(cnt, LOG_ERR, 1, "listen()");
		close(sl);
		return -1;
	}
...
int http_acceptsock(struct context *cnt, int sl)
{
	int sc;
	unsigned long i;
	struct sockaddr_in6 sin;
	...
	motion_log(cnt, LOG_ERR, 1, "accept()");
 ...
 */
struct webcam_buffer *webcam_tmpbuffer(struct context *cnt, int size)
{
	struct webcam_buffer *tmpbuffer=mymalloc(cnt, sizeof(struct webcam_buffer));
	tmpbuffer->ref=0;
	tmpbuffer->ptr=mymalloc(cnt, size);
...
void webcam_add_client(struct context *cnt, struct webcam *list, int sc)
{
	struct webcam *new=mymalloc(cnt, sizeof(struct webcam));
	...
	if ((new->tmpbuffer = webcam_tmpbuffer(cnt, sizeof(header))) == NULL) {
		motion_log(cnt, LOG_ERR, 1, "Error creating tmpbuffer in webcam_add_client");
	} else {
...
{
	cnt->webcam.socket=http_bindsock(cnt, cnt->conf.webcam_port, cnt->conf.webcam_localhost);
...
{	
	...
	if (cnt->conf.setup_mode)
		motion_log(cnt, -1, 0, "Closing webcam listen socket");
	
	close(cnt->webcam.socket);
	
	if (cnt->conf.setup_mode)
		motion_log(cnt, LOG_ERR, 1, "Closing active webcam sockets");
...
void webcam_put(struct context *cnt, unsigned char *image)
{
	...
	if ((cnt->stream_count < DEF_MAXSTREAMS) &&
		...
		sc=http_acceptsock(cnt, sl);
		webcam_add_client(cnt, &cnt->webcam, sc);
		cnt->stream_count++;
	}
	...
	if (webcam_check_write(&cnt->webcam)) {
		 ...
		 */
		tmpbuffer=webcam_tmpbuffer(cnt, cnt->imgs.size);
		...
		if (tmpbuffer) {
			...
			motion_log(cnt, LOG_ERR, 1, "Error creating tmpbuffer");
		}
	}
