...
static char* not_valid_syntax_raw =
	"HTTP/1.0 404 Not Valid Syntax\r\n"
	"Content-type: text/plain\r\n\r\n"
	"Not Valid Syntax\n";
...
int send_template_ini_client(int client_socket, const char* template, void *userdata)
{
	...
	if (nwrite != (strlen(ok_response) + strlen(template)))
		motion_log(cnt[0], LOG_ERR, 1, "httpd send_template_ini_client");
	return nwrite;
}

int send_template_ini_client_raw(int client_socket, void *userdata)
{
	struct context **cnt=userdata;
	int nwrite=0;
	nwrite = write (client_socket, ok_response_raw, strlen (ok_response_raw));
	if (nwrite != strlen (ok_response_raw))
		motion_log(cnt[0], LOG_ERR, 1, "httpd send_template_ini_client_raw");
	return nwrite;
}

int send_template(int client_socket, char *res, void *userdata)
{
	struct context **cnt=userdata;
	int nwrite=0;
	nwrite = write (client_socket, res, strlen(res));
	if (nwrite != (strlen(res)) )
		motion_log(cnt[0], LOG_ERR, 1, "httpd send_template failure write");
	return nwrite;
}
...
int response_client(int client_socket, const char* template, char *back, void *userdata)
{
	struct context **cnt=userdata;
	int nwrite=0;
	nwrite = write (client_socket, template, strlen(template));
	if (back != NULL) {
		send_template(client_socket, back, cnt);
...
int check_authentication(void *userdata, char *authentication, char *auth_base64, size_t size_auth, char *conf_auth)
{
	int ret=0;
	char *userpass = NULL;
	struct context **cnt=userdata;

	authentication = (char *) mymalloc(cnt[0], BASE64_LENGTH(size_auth) + 1);
	userpass = mymalloc(cnt[0], size_auth + 4);
...
int url_decode(char *urlencoded, int length)
{
...
int config(char *pointer, char *res, int length_uri, int thread, int client_socket, void *userdata)
{
	char question;
	char command[256]={'\0'};
	int i;
	...
	if (!strcmp(command,"list")) {
		...
		if (length_uri==0) {
			...
			if (cnt[0]->conf.control_html_output) {
				send_template_ini_client(client_socket, ini_template, cnt);
				sprintf(res, "<a href=/%d/config><- back</a>", thread);
				send_template(client_socket, res, cnt);
				...
				send_template_ini_client_raw(client_socket, cnt);
				for (i=0; config_params[i].param_name != NULL; i++) {
					value=config_params[i].print(cnt, NULL, i, thread);
					if (value == NULL)
						value=config_params[i].print(cnt, NULL, i, 0);
			...
			/*error*/
			if (cnt[0]->conf.control_html_output)
				response_client(client_socket, not_found_response_valid_command, NULL, cnt);
			else
				response_client(client_socket, not_found_response_valid_command_raw, NULL, cnt);
		}
		...
		if ((length_uri!=0) && (question == '?')) {
				...
				if (config_params[i].param_name) {
					if (length_uri > 0) {
						...
						if ( (length_uri==0) && (strlen(Value) > 0) ) {
							...
							if (cnt[0]->conf.control_html_output) {
								sprintf(res,"<li><a href=/%d/config/set?%s>%s</a> = %s</li> <b>Done</b><br>\n"
								            "<a href=/%d/config/list><- back</a>\n", thread,
								        config_params[i].param_name, config_params[i].param_name, Value, thread);
								send_template_ini_client(client_socket, ini_template, cnt);
								send_template(client_socket,res, cnt);
								send_template_end_client(client_socket);
							} else {
							...
							/*error*/
							if (cnt[0]->conf.control_html_output)
								response_client(client_socket, not_valid_syntax, NULL, cnt);
							else
								response_client(client_socket, not_valid_syntax_raw, NULL, cnt);
						}
					} else {
						char *type=NULL;
						...
						if (cnt[0]->conf.control_html_output) {
							sprintf(res, "<li><a href=/%d/config/set?%s>%s</a> = %s</li> <b>Done</b><br>\n"
							             "<a href=/%d/config/list><- back</a>", thread,
							        config_params[i].param_name, config_params[i].param_name, type, thread);
							send_template_ini_client(client_socket, ini_template, cnt);
							send_template(client_socket, res, cnt);
							send_template_end_client(client_socket);
						} else {
			...
			if (cnt[0]->conf.control_html_output) {
				send_template_ini_client(client_socket, set_template, cnt);
				sprintf(res, "<b>Thread %d </b>\n<form name='n'>\n<select name='onames'>\n", thread);
				send_template(client_socket, res, cnt);
				for (i=0; config_params[i].param_name != NULL; i++) {
...
int action(char *pointer, char *res, int length_uri, int thread, int client_socket, void *userdata)
{
	...
	if (!strcmp(command,"makemovie")) {
		...
		if (length_uri==0) {
			int i=0;
			do {
				motion_log(cnt[0], LOG_DEBUG, 0, "httpd restart");
				kill(getpid(),1);
			} while (cnt[++i]);
		...
		if (length_uri==0) {
			...
			/*call quit*/
			do {
				motion_log(cnt[0], LOG_DEBUG, 0, "httpd quitting");
				cnt[i]->makemovie=1;
				cnt[i]->finish=1;
			} while (cnt[++i]);
...
int detection(char *pointer, char *res, int length_uri, int thread, int client_socket, void *userdata)
{
	char command[256]={'\0'};
	struct context **cnt=userdata;
...
int track(char *pointer, char *res, int length_uri, int thread, int client_socket, void *userdata)
{
	char question;
	char command[256]={'\0'};
	struct context **cnt=userdata;
	...
	if (!strcmp(command,"set")) {
		...
		if ((question == '?') && (length_uri>2)) {
			...
			if (( question != '=' ) || (command[0] == '\0')) {
				/* no valid syntax */
				motion_log(cnt[0], LOG_WARNING, 0, "httpd debug race 1");
				if (cnt[0]->conf.control_html_output)
					response_client(client_socket, not_valid_syntax, NULL, cnt);
				else
					response_client(client_socket, not_valid_syntax_raw, NULL, cnt);
			...
			if (!strcmp(command,"pan")) {
				...
				/* no valid syntax */
				motion_log(cnt[0], LOG_WARNING, 0, "httpd debug race 2");
				if (cnt[0]->conf.control_html_output)
					response_client(client_socket, not_valid_syntax, NULL, cnt);
				else
					response_client(client_socket, not_valid_syntax_raw, NULL, cnt);
			...
			warningkill = sscanf (pointer, "%10[-0-9]" , value1);
			if ( !warningkill ) {
				motion_log(cnt[0], LOG_WARNING, 0, "httpd debug race 3");
				/* error value */
				if (cnt[0]->conf.control_html_output)
					response_client(client_socket, error_value, NULL, cnt);
				else
					response_client(client_socket, error_value_raw, NULL, cnt);
				return 1;
			}

			pointer = pointer + strlen(value1);
			length_uri = length_uri - strlen(value1);
				...
				if (pan) {
					...
					/*error in track action*/
						if (cnt[0]->conf.control_html_output) {
							...
							response_client(client_socket, track_error, res, cnt);
						}
						else
							response_client(client_socket, track_error_raw, NULL, cnt);
					}
				} else if (tilt) {
					struct coord cent;
					...
					cent.x=0;
					cent.y=atoi(value1);
					if (track_move(tiltcnt, tiltcnt->video_dev,
					               tiltcnt->pipe, tiltcnt->mpipe, &cent, &tiltcnt->imgs, 1)) {
						if (cnt[0]->conf.control_html_output) {
							send_template_ini_client(client_socket, ini_template, cnt);
			...
			if ( ( question != '&' ) || (command[0] == '\0') ){
				motion_log(cnt[0], LOG_WARNING, 0, "httpd debug race 4");
				if ( strstr(pointer,"&")){
					if (cnt[0]->conf.control_html_output)
						response_client(client_socket, error_value, NULL, cnt);
					else
			...
			if (!strcmp(command,"pan")){
				...
				length_uri=length_uri-3;
				if ( (pan) || (!tilt) || (X) || (Y) ) {
					motion_log(cnt[0], LOG_WARNING, 0, "httpd debug race 5");
					/* no valid syntax */
					if (cnt[0]->conf.control_html_output)
						response_client(client_socket, not_valid_syntax, NULL, cnt);
					else
				...
				if ( (tilt) || (!pan) || (X) || (Y) ) {
					/* no valid syntax */
					motion_log(cnt[0], LOG_WARNING, 0, "httpd debug race 6");
					if (cnt[0]->conf.control_html_output)
						response_client(client_socket, not_valid_syntax, NULL, cnt);
					else
						response_client(client_socket, not_valid_syntax_raw, NULL, cnt);
				...
				length_uri--;
				if ( (X) || (!Y) || (pan) || (tilt) ) {
					motion_log(cnt[0], LOG_WARNING, 0, "httpd debug race 7");
				...
				length_uri--;
				if ( (Y) || (!X) || (pan) || (tilt) ){
					motion_log(cnt[0], LOG_WARNING, 0, "httpd debug race 8");
					/* no valid syntax */
					if (cnt[0]->conf.control_html_output)
						response_client(client_socket, not_valid_syntax, NULL, cnt);
					else
				...
				motion_log(cnt[0], LOG_WARNING, 0, "httpd debug race 9");
				/* no valid syntax */
				if (cnt[0]->conf.control_html_output)
					response_client(client_socket, not_valid_syntax, NULL, cnt);
				else
			...
			warningkill = sscanf (pointer, "%c%10[-0-9]" ,&question, value2);
			
			if ( ( warningkill < 2 ) && (question != '=') ) {
				motion_log(cnt[0], LOG_WARNING, 0, "httpd debug race 10");
				/* no valid syntax */
				if (cnt[0]->conf.control_html_output)
					response_client(client_socket, not_valid_syntax, NULL, cnt);
				else
				...
				motion_log(cnt[0], LOG_WARNING, 0, "httpd debug race 11");
				if (cnt[0]->conf.control_html_output)
					response_client(client_socket, error_value, NULL, cnt);
				else
					response_client(client_socket, error_value_raw, NULL, cnt);
			...
			length_uri = length_uri - strlen(value2) - 1;

			if (length_uri != 0) {
				motion_log(cnt[0], LOG_WARNING, 0, "httpd debug race 12");
				if (cnt[0]->conf.control_html_output)
					response_client(client_socket, error_value, NULL, cnt);
				else
					response_client(client_socket, error_value_raw, NULL, cnt);
			...
			if ( X && Y ) {
				...
				if (track_center(setcnt, setcnt->video_dev, 1, atoi(value1), atoi(value2))) {
					if (cnt[0]->conf.control_html_output){
						send_template_ini_client(client_socket, ini_template, cnt);
						sprintf(res,"track set %s %s<br>\n"
						            "<a href=/%d/track><- back</a>\n", value1, value2, thread);
						send_template(client_socket, res, cnt);
						send_template_end_client(client_socket);
					} else {
						send_template_ini_client_raw(client_socket, cnt);
				...
				cent.y=0;
				cent.x=atoi(value1);
				if (track_move(relativecnt, relativecnt->video_dev, relativecnt->pipe,
				               relativecnt->mpipe, &cent, &relativecnt->imgs, 1)) {
					/* move tilt */
					relativecnt=cnt[thread];
					cent.width=relativecnt->imgs.width;
					cent.height=relativecnt->imgs.height;
			...
			if ((thread != -1) && (thread < i)) {
				...
				if (length_uri!=0) {
					...
					/* detection */
						...
						pointer = pointer + 9;length_uri = length_uri - 9;
						if (length_uri == 0) {
							if (cnt[0]->conf.control_html_output) {
								send_template_ini_client(client_socket, ini_template, cnt);
								sprintf(res,"<b>Thread %d</b><br>\n"
					...
					/* track */
						...
						pointer = pointer + 5;length_uri = length_uri - 5;
						if (length_uri == 0) {
							if (cnt[0]->conf.control_html_output) {
								send_template_ini_client(client_socket, ini_template, cnt);
								sprintf(res,"<b>Thread %d</b><br>\n"
							...
							if (cnt[thread]->track.type) {
								...
								if (cnt[0]->conf.control_html_output) {
									...
									response_client(client_socket, not_track,res, cnt);
								}
								else
									response_client(client_socket, not_track_raw,NULL, cnt);
							}
						} else {
							if (cnt[0]->conf.control_html_output) {
								sprintf(res,"<a href=/%d/><- back</a>\n",thread);
								response_client(client_socket, not_found_response_valid_command, res, cnt);
							}
							else
								response_client(client_socket, not_found_response_valid_command_raw, NULL, cnt);						}
					} else {
				...
				if (cnt[0]->conf.control_html_output) {
					...
					response_client(client_socket, not_found_response_valid, res, cnt);
				}
				else
					response_client(client_socket, not_found_response_valid_raw, NULL, cnt);
			}
		}
		free(res);
	} else {
		if (cnt[0]->conf.control_html_output)
			response_client(client_socket, not_found_response_template,NULL, cnt);
		else
			response_client(client_socket, not_found_response_template_raw,NULL, cnt);
	}
...
int read_client(int client_socket, void *userdata, char *auth)
{
	...
	while (alive)
	{
		...
		if (nread <= 0) {
			motion_log(cnt[0], LOG_ERR, 1, "httpd First read");
			...
			while ((strstr (buffer, "\r\n\r\n") == NULL) && (readb!=0) && (nread < length)){
				...
				if (nread > length) {
					motion_log(cnt[0], LOG_ERR, 1, "httpd End buffer reached waiting for buffer ending");
					break;
				}
				buffer[nread] = '\0';
			}
			...
			if (nread == -1) {
				motion_log(cnt[0], LOG_ERR, 1, "httpd READ");
				pthread_mutex_unlock(&httpd_mutex);
				return -1;
			}
			alive=0;
			...
			if (strcmp (protocol, "HTTP/1.0") && strcmp (protocol, "HTTP/1.1")) {
				...
				if ( (authentication = strstr(buffer,"Basic")) ) {
					...
					if ( !check_authentication(cnt, auth, authentication,
...
int acceptnonblocking(int serverfd, int timeout)
{
	int curfd;
...
void httpd_run(struct context **cnt)
{
	...
	if (sd<0) {
		motion_log(cnt[0], LOG_ERR, 1, "httpd socket");
		return;
	}
	...
	servAddr.sin6_port = htons(cnt[0]->conf.control_port);
	...
	if (bind(sd, (struct sockaddr *) &servAddr, sizeof(servAddr))<0) {
		motion_log(cnt[0], LOG_ERR, 1, "httpd bind()");
		close(sd);
		return ;
	}

	if (listen(sd,5) == -1){
		motion_log(cnt[0], LOG_ERR, 1, "httpd listen()");
		close(sd);
		return;
	}

	motion_log(cnt[0], LOG_DEBUG, 0, "motion-httpd/"VERSION" running, accepting connections");
	motion_log(cnt[0], LOG_DEBUG, 0, "motion-httpd: waiting for data on port TCP %d", cnt[0]->conf.control_port);

	if (cnt[0]->conf.control_authentication != NULL ) {
		char *userpass = NULL;
		size_t auth_size = strlen(cnt[0]->conf.control_authentication);

		authentication = (char *) mymalloc(cnt[0], BASE64_LENGTH(auth_size) + 1);
		userpass = mymalloc(cnt[0], auth_size + 4);
		/* base64_encode can read 3 bytes after the end of the string, initialize it */
	...
	while ((client_sent_quit_message!=0) && (!closehttpd)) { 
		...
		if (client_socket_fd<0) {
			if (cnt[0]->finish){ 
				motion_log(cnt[0], -1, 1, "httpd - Finishing");
				closehttpd = 1;
			}
		} else {
			/* Get the Client request */
			client_sent_quit_message = read_client (client_socket_fd, cnt, authentication);
			motion_log(cnt[0], -1, 1, "httpd - Read from client");
	...
	close(sd);
	motion_log(cnt[0], LOG_DEBUG, 0, "httpd Closing");
