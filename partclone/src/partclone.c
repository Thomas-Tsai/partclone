...
{
    fprintf(stderr, "%s v%s (%s) partclone.sourceforge.net partclone.nchc.org.tw\nUsage: %s [OPTIONS]\n"
...
extern void print_image_hdr_info(image_head image_hdr, cmd_opt opt){
	...
	log_mesg(0, 0, 1, debug, _("Partclone v%s (%s) partclone.sourceforge.net partclone.nchc.org.tw\n"), VERSION, svn_version);
    if (opt.clone)
